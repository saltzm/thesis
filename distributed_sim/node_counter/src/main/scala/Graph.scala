...
case class Graph(adjList: Array[Set[Int]], 
  ...
  def writeToFiles(lFile: String, eFile: String) {
    val out = new PrintWriter(lFile)
    labels.foreach( out.println(_) )
    out.close
    val eOut = new PrintWriter(eFile)
    adjList.foreach{ set => 
      set.foreach( s => eOut.print(s + " ") )
      eOut.println
    }
    eOut.close
  }
...
object GraphWriterTest extends App {
  ...
  g.writeToFiles("gltest.txt", "getest.txt")
}
